...
import { Accommodation, Activity, DaySchedule, LinkInfo } from '@/types/schedule';
...
const isActivityArray = (value: unknown): value is Activity[] => {
  return Array.isArray(value) && value.every((item) =>
    ...
    (((item as Activity).description === undefined) || typeof (item as Activity).description === 'string') &&
    (((item as Activity).emoji === undefined) || typeof (item as Activity).emoji === 'string')
  );
};
...
const isAccommodation = (value: unknown): value is Accommodation | null => {
  ...
  return typeof value === 'object' &&
    ...
    (value as Accommodation).options.every((option) => typeof option === 'string') &&
...
export async function PATCH(
  ...
  if ('activities' in body) {
    if (!isActivityArray(body.activities)) {
      return NextResponse.json({ error: 'activities 형식이 올바르지 않습니다.' }, { status: 400 });
    }
  ...
  if ('accommodation' in body) {
    if (!isAccommodation(body.accommodation)) {
      return NextResponse.json({ error: 'accommodation 형식이 올바르지 않습니다.' }, { status: 400 });
    }
  ...
  if ('links' in body) {
    if (!isLinkArray(body.links)) {
      return NextResponse.json({ error: 'links 형식이 올바르지 않습니다.' }, { status: 400 });
    }
  ...
  if (!Number.isInteger(dayId)) {
    return NextResponse.json({ error: '잘못된 일정 ID입니다.' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: '일정 저장 중 오류가 발생했습니다.' }, { status: 500 });
  }
}
